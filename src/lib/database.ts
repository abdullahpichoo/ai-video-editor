...
const DATABASE_NAME = process.env.MONGODB_DB_NAME || 'ai-video-editor';
...
export async function initializeDatabase(): Promise<void> {
  try {
    ...
    await db.collection('userSessions').createIndexes([
      { key: { userId: 1 } },
    ...
    await db.collection('videoProjects').createIndexes([
      { key: { userId: 1 } },
    ...
    await db.collection('aiProcessingJobs').createIndexes([
      { key: { userId: 1 } },
    ...
    await db.collection('projectExports').createIndexes([
      { key: { userId: 1 } },
